...
public class tromcomp {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (Path javaFile : javaFiles) {
            ...
            int tassertValue = Tassert.getCountAssertExp();
        ...
        for (int i = 0; i < javaFiles.size(); i++) {
            if (selectedTlocValues.contains(tlocV.get(i)) && selectedTassertValues.contains(tassertV.get(i))) {
                String javaFilePath = javaFiles.get(i).toString();
                String packageName = Tls.getPackageName();
                String className = Tls.getClassName();

                String[] fileInfo = {
                        javaFilePath,
                        packageName,
                        className,
                        String.valueOf(tlocV.get(i)),
                        String.valueOf(tassertV.get(i)),
                        String.valueOf((double) tlocV.get(i) / tassertV.get(i))
                };

                suspiciousFilesInfo.add(fileInfo);

                // Afficher les résultats sur la console
                String line = String.join(", ", fileInfo);
                System.out.println(line);
            }
        }

        // Ecrire les résultats au fichier CSV spécifié
        if (!outputFile.isEmpty()) {
            Tls.givenDataArray_whenConvertToCSV_thenOutputCreated(suspiciousFilesInfo, Paths.get(outputFile));
